...
val baseApplicationId = "com.flexplayer.music"
val applicationIdOverride = System.getenv("flex-player_APPLICATION_ID")?.takeIf { it.isNotBlank() }
val appNameOverride = System.getenv("flex-player_APP_NAME")?.takeIf { it.isNotBlank() }
val buildCommit =
    System.getenv("flex-player_BUILD_COMMIT")
        ?.trim()
        ?.takeIf { it.matches(Regex("[0-9a-fA-F]{7,40}")) }
        ?.take(7)
        ?.lowercase()
val debugKeystorePathOverride = System.getenv("flex-player_DEBUG_KEYSTORE_PATH")?.takeIf { it.isNotBlank() }
val debugKeystorePassword = System.getenv("flex-player_DEBUG_KEYSTORE_PASSWORD")?.takeIf { it.isNotBlank() } ?: "android"
val debugKeyAlias = System.getenv("flex-player_DEBUG_KEY_ALIAS")?.takeIf { it.isNotBlank() } ?: "androiddebugkey"
val debugKeyPassword = System.getenv("flex-player_DEBUG_KEY_PASSWORD")?.takeIf { it.isNotBlank() } ?: "android"
...
android {
    namespace = "com.flexplayer.music"
    compileSdk = 37
    ...
    defaultConfig {
        ...
        resValue("string", "app_name", appNameOverride ?: "flex-player")
    ...
    buildTypes {
        ...
        debug {
            ...
            if (appNameOverride == null) {
                resValue("string", "app_name", "flex-player Debug")
            }
            signingConfig =
...
val cleanLegacyProtoSources = tasks.register<Delete>("cleanLegacyProtoSources") {
    delete(layout.projectDirectory.dir("src/main/java/com/flex-player/music/listentogether/proto"))
}
