...
import com.flexplayer.innertube.pages.ArtistSection
...
fun serializeArtistPage(
    ...
    artist: com.flexplayer.innertube.models.ArtistItem? = null,
): String {
...
fun CachedArtistPage.toArtistPage(): com.flexplayer.innertube.pages.ArtistPage {
    return com.flexplayer.innertube.pages.ArtistPage(
        artist = artist?.let { cached ->
            com.flexplayer.innertube.models.ArtistItem(
                id = cached.id,
                title = cached.title,
        ...
        sections = sections.map { section ->
            ArtistSection(
                ...
                moreEndpoint = section.moreBrowseId?.let { browseId ->
                    com.flexplayer.innertube.models.BrowseEndpoint(
                        browseId = browseId,
...
private fun CachedItem.toYTItem(): YTItem {
    return when (this) {
        is CachedItem.Song -> SongItem(
            ...
            thumbnail = thumbnail,
            explicit = explicit,
            artists = artists.map { com.flexplayer.innertube.models.Artist(it.name, it.id) },
            album = album?.let { com.flexplayer.innertube.models.Album(it.name, it.id) },
            duration = duration,
            setVideoId = videoId,
        )
        is CachedItem.Album -> AlbumItem(
            ...
            thumbnail = thumbnail,
            explicit = explicit,
            artists = artists.map { com.flexplayer.innertube.models.Artist(it.name, it.id) }.ifEmpty { null },
            year = year,
        )
        is CachedItem.Playlist -> PlaylistItem(
            ...
            author = author?.let { com.flexplayer.innertube.models.Artist(it.name, it.id) },
        ...
        is CachedItem.Episode -> EpisodeItem(
            ...
            explicit = explicit,
            author = author?.let { com.flexplayer.innertube.models.Artist(it.name, it.id) },
            podcast = album?.let { com.flexplayer.innertube.models.Album(it.name, it.id) },
            duration = duration,
            endpoint = null,
        )
    }
